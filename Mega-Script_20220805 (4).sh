...
{   
    ...
    PYTHON_VERSION=`python3 -V | cut -d " " -f 2 | cut -c 1-4`
...
#Script to check compliance status of a mac
    ...
    if [[ "$(command Hostname)" == "MX0"* ]]  || [[ "$(command Hostname)" == "AR0"* ]] || [[ "$(command Hostname)" == "CO0"* ]] || [[ "$(command Hostname)" == "BR0"* ]]  || [[ "$(command Hostname)" == "CL0"* ]] || [[ "$(command Hostname)" == "UY0"* ]]; then
